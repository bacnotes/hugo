...
- 參數為 primitives，直接傳值(flightNum)，若為 objects 則會傳址(passenger)
- 下方 checkIn 函式會把 passenger 裡面的屬性 name 加上一個稱謂
...
- step3. 執行個別文本
  ...
  - 因為 booker 是全域下的變數，所以可取得全域的 secureBooking=fn, booker=fn 的變數環境，而 secureBooking()的變數環境裡面有 passengerCount=0
...
註：如果想知道這個神秘的 Closure 變數存在哪裡的話，可以在 devtools 裡 console.dir 偷偷看一下匿名函式的[[Scopes]]下 Closure 的 passengerCount 變數。但因為是 private 作用域所以無法存取或修改。
