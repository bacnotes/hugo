---
title: JavaScript基礎知識(2)｜The Complete JavaScript Course 2022｜bacnotes備份筆記
description: JavaScript是一個可以開發前後端的程式語言，使用的時候總是會不小心踩到一些坑嗎？透過The Complete JavaScript Course 2022這堂課，讓我對於JavaScript有了更深的理解，這篇會介紹關於JavaScript的基礎知識（後半篇）。
...
如果還沒看過
[JavaScript基礎知識(1)｜The Complete JavaScript Course 2022](https://bacnotes.github.io/p/javascript-fundamentals-part1/ "JavaScript基礎知識(1)｜The Complete JavaScript Course 2022")   
可以看完上一篇，再回來繼續閱讀
...
```
- 使用到reserved keywords

## function
- 實踐dry code，重複使用一樣的邏輯，傳不同的參數
- parameter是function的右邊的參數 
- argument是用在實際呼叫的值
...
```
### 函式表達式
- 匿名函式是expression會產生value，存到左邊變數
```
...
arrayA[2] = 234 
```
- const不是不能修改值嗎？ 單獨修改元素值是可以的
- 如果是arrayA = ['a', 'b', 'c'] 就不行，因為就表示重新賦值整個陣列
- 補充immutable（不可變更）跟mutable（可變更）：只有賦值時是傳值的primitives是immutable，而賦值時是傳址的陣列、物件內部值是mutable
...
```
### brackets notation可以使用expression，dot notation無法
```
const interestedIn = prompt('what do you want ot know about Jonas? Choose between firstName, lastName,age or job')
...
jonas[interestedIn] ? console.log(jonas[interestedIn]) : console.log('data not existed')
```
### 新增/修改元素值 
```
...
- 如果需要用一樣的物件，寫this就不需要修改裡面的寫死的變數
```
const jonas = {
  firstName: 'Jona',
  lastName: 'Schmedtmann',
...
- 適合確定迴圈次數時使用(e.g. by array的元素長度或一個數字)
- 可用來精簡重複執行重複的程式碼
```
// 假設要執行10次，從0開始到9結束
...
- 適合不確定迴圈次數時使用(e.g. Math.random的亂數)
```
while (i < 10) {
